...
void ml2c_Unix_struct_statfs(value v,struct statfs * st)
{
  ...
  st->f_namelen = Int64_val(Field(v,6));
}
...
#define readdir_CB vres=callback2(*readdir_closure,vpath,Val_int(info->fh));
#define readdir_RES \
  vtmp=Field(vres,0);	    \
  while (Is_block(vtmp))    \
    { \
...
#define mknod_CB vres=callback2(*mknod_closure,vpath,Val_int(mode));
#define mknod_RES
...
#define open_CB vres=callback2(*open_closure,vpath,c_flags_to_open_flag_list(fi->flags));
...
#define opendir_CB vres=callback2(*opendir_closure,vpath,c_flags_to_open_flag_list(fi->flags));
...
#define release_CB vres=callback3(*release_closure,vpath,c_flags_to_open_flag_list(fi->flags),Val_int(fi->fh));
#define release_RES

#define releasedir_ARGS (const char *path, struct fuse_file_info * fi) 
#define releasedir_CB vres=callback3(*releasedir_closure,vpath,c_flags_to_open_flag_list(fi->flags),Val_int(fi->fh));
#define releasedir_RES

#define flush_ARGS (const char *path,struct fuse_file_info * fi)
#define flush_CB vres=callback2(*flush_closure,vpath,Val_int(fi->fh));
#define flush_RES 
...
#define fsync_CB vres=callback3(*fsync_closure,vpath,Val_bool(isdatasync),Val_int(fi->fh));
#define fsync_RES

#define fsyncdir_ARGS (const char *path, int isdatasync,struct fuse_file_info * fi) 
#define fsyncdir_CB vres=callback3(*fsync_closure,vpath,Val_bool(isdatasync),Val_int(fi->fh));
#define fsyncdir_RES
